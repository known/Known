...
/// 报表类型表单。
/// </summary>
public class ReportTypeForm : AntForm<ReportInfo> { }
...
public partial class ReportSetting
{
    ...
    public ReportInfo Report
    {
        ...
        set
        {
            model = value ?? new ReportInfo();
            LoadConfig();
        }
    }

    [Parameter] public Func<ReportInfo, Task<Result>> OnSave { get; set; }

    protected override async Task OnInitAsync()
    {
        await base.OnInitAsync();
        model ??= new ReportInfo();
        LoadConfig();
    }

    private void LoadConfig()
    {
        reportConfig = !string.IsNullOrWhiteSpace(model.Config)
            ? Utils.FromJson<ReportConfig>(model.Config) ?? new ReportConfig()
            : new ReportConfig();
        chartConfig = reportConfig.Chart ?? new ChartConfig();
        tableConfig = reportConfig.Table ?? new TableConfig();
    }

    public void SaveConfig()
    {
        reportConfig.Chart = chartConfig;
        reportConfig.Table = tableConfig;
        model.Config = Utils.ToJson(reportConfig);
    }
