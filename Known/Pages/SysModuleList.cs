...
public class SysModuleList : KDataGrid<SysModule, SysModuleForm>
{
    ...
    protected override void BuildPage(RenderTreeBuilder builder)
    {
        builder.ViewLR(left =>
        {
            builder.Component<KTree<string>>()
                   .Set(c => c.Data, data)
                   .Set(c => c.OnItemClick, Callback<KTreeItem<string>>(OnTreeItemClick))
                   .Build();
        }, right => base.BuildPage(builder));
    }

    protected override Task<PagingResult<SysModule>> OnQueryDataAsync(PagingCriteria criteria)
    {
    ...
    private void OnTreeItemClick(KTreeItem<string> item)
    {
        current = item;
        RefreshGridData();
    }
    ...
    {
        data.Clear();
        var root = new KTreeItem<string> { Value = "0", Text = Config.AppName, Icon = "fa fa-tv", IsExpanded = true };
        data.Add(root);
        current = root;
    ...
    private async void RefreshTreeNode(KTreeItem<string> item, bool reload = false)
    {
        if (reload)
            datas = await Platform.Module.GetModulesAsync();
        ...
        {
            var sub = new KTreeItem<string> { Value = child.Id, Text = child.Name, Icon = child.Icon };
            item.AddChild(sub);
            RefreshTreeNode(sub);
        }
    }
