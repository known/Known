...
class SysMessageList : KDataGrid<SysMessage>
{
    ...
    protected override Task<PagingResult<SysMessage>> OnQueryDataAsync(PagingCriteria criteria)
    {
...
class SysMessageForm : BaseForm<SysMessage>
{
    ...
    protected override void BuildFields(FieldBuilder<SysMessage> builder)
    {
        ...
        builder.Table(table =>
        {
            ...
            table.Tr(attr => builder.Field<KText>(f => f.Subject).ColSpan(5).ReadOnly(true).Build());
            table.Tr(attr =>
            {
                table.Th("", "级别");
                table.Td(attr => SysMessageList.BuildMsgLevel(table.Builder, model.MsgLevel));
                table.Field<KText>(f => f.MsgBy).ReadOnly(true).Build();
                table.Field<KDate>(f => f.CreateTime)
                     .Label("收件时间").ReadOnly(true)
                     .Set(f => f.DateType, DateType.DateTime)
                     .Build();
            });
            table.Tr(attr => table.Field<KText>(f => f.Content).ColSpan(5).ReadOnly(true).Build());
        });
    }
