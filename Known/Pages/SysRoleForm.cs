...
class SysRoleForm : BaseForm<SysRole>
{
    ...
    private List<KTreeItem<MenuInfo>> data;
    private KTreeItem<MenuInfo> curItem;
    ...
    private void BuildRoleModules(RenderTreeBuilder builder)
    {
        builder.Div("role-module", attr =>
        {
            ...
            builder.Component<KTree<MenuInfo>>()
                   .Set(c => c.Data, data)
                   .Set(c => c.ReadOnly, ReadOnly)
                   .Set(c => c.ShowCheckBox, true)
                   .Set(c => c.Values, values)
                   .Set(c => c.OnCheckClick, Callback<KTreeItem<MenuInfo>>(OnMenuItemCheck))
                   .Set(c => c.OnItemClick, Callback<KTreeItem<MenuInfo>>(OnMenuItemClick))
                   .Set(c => c.ValuesChanged, Callback<HashSet<MenuInfo>>(v => values = v))
                   .Build();
        });
    }

    private void OnMenuItemCheck(KTreeItem<MenuInfo> item) => SetCurItem(item, true);
    private void OnMenuItemClick(KTreeItem<MenuInfo> item) => SetCurItem(item);

    private void SetCurItem(KTreeItem<MenuInfo> item, bool isCheck = false)
    {
        curItem = item;
