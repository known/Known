...
class SqlServerProvider(Database db) : DbProvider(db)
{
    ...
    public override string GetBackupTableSql(string tableName, string suffix)
    {
        return $"SELECT * INTO {tableName}{suffix} FROM {tableName}";
    }
    ...
    internal override string GetAddFieldScript(string tableName, List<FieldInfo> fields)
    {
        ...
        foreach (var item in fields)
        {
            ...
            sb.AppendLine($"ALTER TABLE {tableName} ADD {item.Id} {type} NULL;");
        }
        return sb.ToString();
    }
    ...
    internal static string GetTableScript(string tableName, List<FieldInfo> fields, List<string> keys, int maxLength = 0)
    {
        ...
        foreach (var item in fields)
        {
            ...
            var identity = item.IsKey && item.IsAutoKey ? " IDENTITY(1,1)" : "";
            var column = $"[{item.Field ?? item.Id}]";
            column = GetColumnName(column, maxLength + 2);
            var type = GetSqlServerDbType(item);
            sb.AppendLine($"    {column} {type}{identity} {required}{comma}");
        }
        if (hasKey)
        {
