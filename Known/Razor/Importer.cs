...
class Importer : BaseComponent
{
    ...
    private void BuildFields(RenderTreeBuilder builder)
    {
        ...
        {
            builder.Field<KUpload>("Upload").Required(true).Enabled(isFinished)
                   ...
                   .Build();
            builder.Button("导入", "fa fa-check", Callback(OnImportAsync), StyleType.Primary, isFinished);
        });
        builder.Div(attr =>
        {
    ...
    private async void OnFilesChanged(List<IBrowserFile> files)
    {
        ...
        if (Option.Action != null)
        {
            ...
            var stream = file.OpenReadStream(KUpload.MaxLength);
    ...
    private async Task OnImportAsync()
    {
        ...
        {
            await form.SubmitFilesAsync(Platform.File.UploadFilesAsync, result =>
            {
                if (!result.IsValid)
                {
                    message = result.Message;
