...
public class FieldBuilder<T>
{
    ...
    public void Hidden(Expression<Func<T, object>> selector) => Field<KHidden>(selector).Build();
    ...
    public FieldAttrBuilder<TField> Field<TField>(string label, string id, bool required = false) where TField : Field
    {
        ...
        if (typeof(TField) == typeof(KSelect))
            builder.Add(nameof(KSelect.EmptyText), "请选择");
        return builder;
    }
    ...
    public void Table(Action<FieldBuilder<T>> child = null)
    {
        ...
        Builder.Cascading(table, b =>
        {
            var fb = new FieldBuilder<T>(b);
            b.Element("table", attr =>
            {
...
public class FieldAttrBuilder<T> : ComponentBuilder<T> where T : Field
{
    ...
    {
        Set(r => r.Value, value?.ToString(Config.DateFormat)).Add(nameof(KDate.Value), value);
        return this;
    }
