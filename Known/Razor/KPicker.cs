...
public class KPicker : Field
{
    ...
    private void BuildInputPicker(RenderTreeBuilder builder)
    {
        ...
        {
            builder.Field<KHidden>(Id).Value(Value);
            builder.Field<KText>(TextField, Required).IsInput(true).Value(text).Enabled(Enabled && CanEdit).Build();
        }
