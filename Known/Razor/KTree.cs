...
public class KTree<TValue> : BaseComponent
{
    private KTreeItem<TValue> CurrentSelected;
    private IEnumerable<KTreeItem<TValue>> dataView = new List<KTreeItem<TValue>>();
    private readonly HashSet<KTreeItem<TValue>> SelectedList = new();
    ...
    [Parameter] public IEnumerable<KTreeItem<TValue>> Data { get; set; }
    [Parameter] public bool ShowCheckBox { get; set; }
    [Parameter] public RenderFragment<KTreeItem<TValue>> ItemSlot { get; set; }
    [Parameter] public EventCallback<KTreeItem<TValue>> OnItemClick { get; set; }
    [Parameter] public EventCallback<KTreeItem<TValue>> OnCheckClick { get; set; }
    [Parameter] public EventCallback<KTree<TValue>> OnChange { get; set; }
    ...
    private void BuildItem(RenderTreeBuilder builder, KTreeItem<TValue> node)
    {
    ...
    private void BuildChildren(RenderTreeBuilder builder, KTreeItem<TValue> node)
    {
    ...
    private static string GetItemCls(KTreeItem<TValue> item)
    {
    ...
    private static string GetListCls(KTreeItem<TValue> item)
    {
    ...
    private static string GetCheckCls(KTreeItem<TValue> item)
    {
    ...
    private bool? GetItemChecked(KTreeItem<TValue> item)
    {
    ...
    //选中值
    private void CheckValues(KTreeItem<TValue> item, HashSet<TValue> values, int level)
    {
    ...
    //单击节点
    private async Task ItemClick(KTreeItem<TValue> item, bool isIconClick)
    {
        if (item.Disabled)
            return;
    ...
    //选中子节点
    private void CheckChildren(KTreeItem<TValue> item)
    {
    ...
    //取消选中子节点
    private void UnCheckChildren(KTreeItem<TValue> item)
    {
        if (item.Disabled) return;
        item.IsChecked = false;
    ...
    //选中CheckBox
    private async Task Check(KTreeItem<TValue> item, bool cked)
    {
        if (item.Disabled) return;
        if (IsIsolated || item.Children.Count == 0)
        {
    ...
    public HashSet<KTreeItem<TValue>> GetSelectedNodes() => SelectedList;
    ...
    public static List<KTreeItem<TValue>> BuildDataFromJson(string json)
    {
        var list = Utils.FromJson<List<KTreeItem<TValue>>>(json);
...
public class KTreeItem<TValue>
{
    ...
    public KTreeItem<TValue> Parent { get; set; }
    public List<KTreeItem<TValue>> Children { get; set; } = new();

    public void AddChild(KTreeItem<TValue> item)
    {
        item.Parent = this;
        Children.Add(item);
    }
    ...
    {
        var item = new KTreeItem<TValue>() { Value = value, Text = text };
        AddChild(item);
    }
