...
partial class TableModel<TItem>
{
    ...
    /// <param name="onSave">编辑保存方法委托。</param>
    /// <param name="row">编辑行绑定的对象。</param>
    public void EditForm<T>(Func<T, Task<Result>> onSave, T row) where T : class, new()
    {
    ...
    {
        EditForm<TItem>(onSave, row, action);
    }
    ...
    public void EditForm<T>(Func<T, Task<Result>> onSave, T row, string action) where T : class, new()
    {
        ...
        isShowEdit = true;
        var isShow = ShowForm(new FormModel<T>(this, IsAuto)
        {
            Action = action,
            Data = row,
            OnSave = onSave
    ...
    {
        EditForm<TItem>(onSave, row, Language.Edit);
    }
    ...
    /// <param name="onSave">编辑保存方法委托。</param>
    /// <param name="row">编辑行绑定的对象。</param>
    public void EditForm<T>(Func<UploadInfo<T>, Task<Result>> onSave, T row) where T : class, new()
    {
        EditForm(onSave, row, Language.Edit);
    }
