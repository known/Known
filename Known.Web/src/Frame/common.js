...
function WsUtil() {
    ...
    this.connectById = function (id, data, callback) {
        window[id] = {};
        this.connect(baseUrl + '/ws?id=' + id, data, function (res) {
            if (res && res.length) {
    ...
    this.connect = function (url, data, callback) {
        ...
        log(Language.Connecting + '......');
        ws = new WebSocket(url);
        ws.onopen = function () {
            log(Language.ClientConnected);
        }
        ...
        ws.onclose = function () {
            if (!isClosed) {
                log(Language.ClientDisconnected);
...
var Utils = {
    ...
    addJs: function (src) {
        ...
        if (item.length) {
            script.src = baseUrl + '/static' + src;
        } else {
            item = $('script[src*="libs/"]');
            if (!item.length) {
                item = $('script[src*="js/"]');
            }
    ...
    getUser: function () {
        var value = sessionStorage.getItem(this.keyUser);
    ...
    getCodes: function (data) {
        ...
        var value = localStorage.getItem(this.keyCode);
        if (!value)
            return [];
...
var Page = {
    complete: function () {
        ...
        // bind date picker
        $('[placeholder="' + DateFormat + '"]:not([readonly])').each(function () {
...
$(document).ajaxSend(function (evt, xhr, settings) {
    xhr.setRequestHeader('Known-Token', curUser ? curUser.Token : '');
    xhr.setRequestHeader('Known-Client', Utils.getClientId());
});
