layui.define('common', function (exports) {
    ...
    var Menu = {
        ...
        show: function (option) {
            this.option = option;
            this.treeData = Utils.list2Tree(option.data, 'pid', '');
        ...
        render: function (obj, pid) {
            ...
            if (pid) {
                ...
                data = getMenu(parent[0].children);
            }

            function getMenu(menus) {
                return menus.filter(function (d) { return d.type !== '按钮'; });
            }
            ...
            $(data).each(function (i, d) {
                ...
                if (pid && d.children) {
                    var child = getMenu(d.children);
                    if (child.length) {
                        html += '  <dl class="layui-nav-child">';
                        $(child).each(function (ci, cd) {
    ...
    var Tab = {
        ...
        addTabByUrl: function (url) {
            var menu = this.option.menus
                ? this.option.menus.filter(function (m) { return m.url === url; })
                : null;
            if (!menu)
                return;
    ...
    exports('admin', {
        ...
        show: function (option) {
            ...
            $.get(option.url.GetUserData, function (result) {
