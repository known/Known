namespace Known.Weixin.Services;
...
class WeixinService(Context context) : ServiceBase(context), IWeixinService
{
    ...
    public async Task<WeixinInfo> GetWeixinAsync(string userId)
    {
        ...
        var info = await ConfigHelper.GetConfigAsync<WeixinInfo>(database, KeyWeixin);
    ...
    {
        await ConfigHelper.SaveConfigAsync(Database, KeyWeixin, model);
